...
res <- explor::prepare_results(res.mca.2)
explor::MCA_ind_plot(res, xax = 1, yax = 2, ind_sup = FALSE, lab_var = NULL,
                     ind_lab_min_contrib = 0, col_var = "age_young_cl_40_bin", labels_size = 9,
                     point_opacity = 0.5, opacity_var = NULL, point_size = 64, ellipses = TRUE,
                     transitions = FALSE, labels_positions = NULL, xlim = c(-1.11, 1.14), ylim = c(-1.06,
                                                                                                   1.19))
...
data.active.3 <- data_v2 %>% select(pf_discussion, fertil_preserv, age_young_cl, bmi_2cl,nb_child_3cl, center_curie, neo_ct,grade_2cl) %>% drop_na()



res.mca.3 <- MCA (data.active.3,ncp = 3, graph = FALSE)

plot(res.mca.3)

################## A garder : plot axe 1 et 2 

#axe 1 et 2 
plot(res.mca.3, axes = c(1, 2),choix = "ind", invisible = "ind")


############### A garder plot ellipse axe 1 et 2 
plotellipses(res.mca.3, axes = c(1, 2), means = FALSE)

#############  A garder plot ellipse axe 2 et 3 

plotellipses(res.mca.3, axes = c(2, 3), means = FALSE)

plotellipses(res.mca.3, axes = c(1, 3), means = FALSE)


# Valeurs propres 
eig.val.3 <- get_eigenvalue(res.mca.3)
head(eig.val.3)


# head(eig.val.3)
# eigenvalue variance.percent cumulative.variance.percent
# Dim.1  0.3260944        23.715958                    23.71596
# Dim.2  0.1521482        11.065321                    34.78128
# Dim.3  0.1447120        10.524510                    45.30579
# Dim.4  0.1292355         9.398944                    54.70473
# Dim.5  0.1240420         9.021235                    63.72597
# Dim.6  0.1225409         8.912065                    72.63803
# 



################### A garder graphiques : plot de décision valeurs propres 

#graphique valeurs propres et règle de décision 

fviz_screeplot (res.mca.3, addlabels = TRUE, ylim = c (0, 45))

# On garde deux axes 

# concernant la qualité de la représentation 
var3 <- get_mca_var(res.mca.3)
var3
...
# 
# Dim 1       Dim 2       Dim 3
# pf_discussion_No   -0.61601758 -0.03528891 -0.13933193
# pf_discussion_Yes   1.14403265  0.06553655  0.25875931
# fertil_preserv_No  -0.42719805 -0.10389366 -0.05341554
# fertil_preserv_Yes  1.65833573  0.40330372  0.20735322
# [0 -30)             1.56205027  1.05220091 -1.76969283
# [30 -35)            1.04918335 -0.26041397  0.79663952
# [35 -40)           -0.05036293 -0.43943481  0.35469581
# 40+                -0.72655440  0.29776377 -0.35169660
# <25                 0.08053703  0.14442896 -0.03499852
# >25                -0.19686829 -0.35304856  0.08555193
# 0                   1.00773543  0.38319368 -0.44469730
# 1                   0.09471481 -0.69379957  0.36142189
# More than 1        -0.57886538  0.06749139  0.09654335
# Curie Paris         0.06710774 -0.12769367 -0.52520191
# Curie St Cloud     -0.12641225  0.24053923  0.98933383
# neo_ct_No          -0.26008693  0.52733300  0.17946122
# neo_ct_Yes          0.30770848 -0.62388693 -0.21232032
# Grade I-II         -0.21417635  0.74867896  0.18532552
# Grade III           0.16360271 -0.57189275 -0.14156445


var3$cos2
# 
# Dim 1       Dim 2       Dim 3
# pf_discussion_No   0.704744223 0.002312713 0.036053435
# pf_discussion_Yes  0.704744223 0.002312713 0.036053435
# fertil_preserv_No  0.708437786 0.041900698 0.011075883
# fertil_preserv_Yes 0.708437786 0.041900698 0.011075883
# [0 -30)            0.188670803 0.085607542 0.242164492
# [30 -35)           0.257397653 0.015857341 0.148397129
# [35 -40)           0.001271283 0.096785256 0.063056870
# 40+                0.359073725 0.060310241 0.084136353
# <25                0.015855188 0.050990436 0.002994191
# >25                0.015855188 0.050990436 0.002994191
# 0                  0.391461556 0.056602126 0.076229847
# 1                  0.002299531 0.123387572 0.033483611
# More than 1        0.359740219 0.004890244 0.010006416
# Curie Paris        0.008483240 0.030715337 0.519600014
# Curie St Cloud     0.008483240 0.030715337 0.519600014
# neo_ct_No          0.080030955 0.328996162 0.038103263
# neo_ct_Yes         0.080030955 0.328996162 0.038103263
# Grade I-II         0.035039831 0.428164071 0.026235505
# Grade III          0.035039831 0.428164071 0.026235505
# 

### Interprétation : les mieux représentée sur l'axe 1  pf discussion, fertil_preserv, age et nombre enfant 

##### axe 2  : neo_ct et grade 
...
# var3$contrib
# Dim 1       Dim 2       Dim 3
# pf_discussion_No    9.45510170  0.06650176  1.08998399
# pf_discussion_Yes  17.55947458  0.12350327  2.02425598
# fertil_preserv_No   5.56263269  0.70514201  0.19597282
# fertil_preserv_Yes 21.59352691  2.73728356  0.76074487
# [0 -30)             6.71312867  6.52843323 19.41643676
# [30 -35)            7.99678823  1.05588843 10.38903726
# [35 -40)            0.03246141  5.29676701  3.62824161
# 40+                 8.19190561  2.94896111  4.32537525
# <25                 0.17644885  1.21622196  0.07508716
# >25                 0.43131942  2.97298702  0.18354640
# 0                  10.83071440  3.35642847  4.75260620
# 1                   0.07016189  8.06882300  2.30214768
# More than 1         6.65020646  0.19375519  0.41683435
# Curie Paris         0.11276522  0.87507516 15.56400475
# Curie St Cloud      0.21241821  1.64839739 29.31824151
# neo_ct_No           1.40524253 12.38114429  1.50762820
# neo_ct_Yes          1.66254046 14.64811437  1.78367280
# Grade I-II          0.76148663 19.94284731  1.28477921
# Grade III           0.58167613 15.23372547  0.98140318


####  contribution la plus importante : axe 1 : pf_discusison, fertil_preserv, age, enfant

##### contribue le plus à l'axe 2 : neo_ct et grade

################################## A mettre dans le rapport : corr plot 
...
# Cos2 des variable sur Dim.1 
fviz_cos2(res.mca.3, choice = "var", axes = 1)

#axe 1 : pf_discussion, fertil_preserv, age

# Cos2 des variable sur Dim.2
fviz_cos2(res.mca.3, choice = "var", axes = 2)

#grade, neo_ct, center

# Cos2 des variable sur Dim.3
fviz_cos2(res.mca.3, choice = "var", axes = 3)
...
#########################   A garder dans le rapport  : Qualité de représentation des variables 


fviz_mca_var(res.mca.3, col.var = "cos2",
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
             repel = TRUE, 
             ggtheme = theme_minimal())
...
############################@  A garder dans le rapport : Graphique récapitulatif de projection des variables ur l'axe 1 et 2 
...
res <- explor::prepare_results(res.mca.3)
explor::MCA_var_plot(res, xax = 1, yax = 2, var_sup = FALSE, var_sup_choice = ,
                     var_lab_min_contrib = 0, col_var = "Variable", symbol_var = NULL, size_var = NULL,
                     size_range = c(10, 300), labels_size = 10, point_size = 56, transitions = FALSE,
                     labels_positions = NULL, labels_prepend_var = FALSE)
...
explor::MCA_ind_plot(res, xax = 1, yax = 2, ind_sup = FALSE, lab_var = NULL,
                     ind_lab_min_contrib = 0, col_var = "pf_discussion", labels_size = 9, point_opacity = 0.5,
                     opacity_var = "Contrib", point_size = 64, ellipses = TRUE, transitions = FALSE,
                     labels_positions = NULL, xlim = c(-1.15, 1.77), ylim = c(-1.36, 1.56))
...
explor::MCA_biplot(res, xax = 1, yax = 2, col_var = "Variable", ind_point_size = 32,
                   ind_opacity = 0.5, ind_opacity_var = NULL, ind_labels = FALSE, var_point_size = 96,
                   var_sup = FALSE, ind_sup = FALSE, labels_size = 12, bi_lab_min_contrib = 0,
                   symbol_var = "Nature", transitions = TRUE, labels_positions = NULL)

res <- explor::prepare_results(res.mca.3)
explor::MCA_var_plot(res, xax = 1, yax = 2, var_sup = FALSE, var_sup_choice = ,
                     var_lab_min_contrib = 0, col_var = "Variable", symbol_var = NULL, size_var = "Contrib",
                     size_range = c(52.5, 700), labels_size = 10, point_size = 56, transitions = FALSE,
                     labels_positions = NULL, labels_prepend_var = FALSE, xlim = c(-0.99, 0.729),
                     ylim = c(-0.964, 0.755))
