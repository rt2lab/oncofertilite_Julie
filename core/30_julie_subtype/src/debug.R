...
opt            <- NULL
opt$db_name    <- "30_julie_subtype" #TODO : the name of your database
opt$class_name <- "MappingSubtypeJulie" #TODO : the name of your class
opt$output_folder <- ""
Sys.setenv("PROJECT_PATH"="/Users/beatrizgrandal/GitHub/databases") #TODO : your PROJECT_PATH
...
load(file.path(output_folder, "30_julie_subtype_mapped.RData"), verbose=TRUE)
self <- db
...
#In this case, use the mapped database (because of dependencies which may exist with previous family var)
load(file.path(output_folder, "30_julie_subtype_preprocessed.RData"), verbose=TRUE)
head(database_preprocessed)


load(file.path(output_folder, "30_julie_subtype_preprocessed_labels.RData"), verbose=TRUE)
