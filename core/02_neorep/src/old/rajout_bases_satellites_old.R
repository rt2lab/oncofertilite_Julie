...
# patient_char 
  ...
  ## Tabac
  load("/Users/ahamypet/RT2Lab/tabac_NAC/data/processed/neo_tabac.RData")
  items_tabac <- neo_tabac %>% select(numdos7, Tabac,actif_at_diag,sevre,quantite,date.sevrage,
                                      tabac_3_classes, tabac_2_classes_ever_as_no, 
                                      tabac_2_classes_ever_as_yes, quantite_tabac_num,tabac_PA ) %>% colnames()
  
  neo_tabac_part <- neo_tabac %>% select(one_of(items_tabac))
  head(neo_tabac_part)
  nrow(neo_tabac_part) # 956
  neo_tabac_part %>% group_by(tabac_3_classes) %>% count()
    # tabac_3_classes     n
    # <chr>           <int>
    # 1 current           179
    # 2 ever              154
...
## comedication and comorbidity
  ...
  load("/Users/ahamypet/RT2Lab/embols/data/processed/neorep_2019_typ_NAC_refined_new.RData")
...
# bc_biology & tumor_char_neo
      ## tils 
            ...
            load("/Users/ahamypet/RT2Lab/collab_Vassili/signatures_Th/data/processed/base_test_charlotte.RData")
            ...
            #  RCB
            load("/Users/ahamypet/RT2Lab/RCB_validation/data/processed/neorep_RCB.RData")
            head(neorep_RCB)
...
# events_and_censor (all integrated in update_survival_preprocessing_delays.r )
            ...
            neorep_new_tmp <-  neorep_2019 %>% 
                 rename(ev_recloc     = status_rec,
                        ...
                        dat_last_news =   date_der_actu)  

# @ASHP @EVA Recheck rec loc and rec reg ??       
# @ASHP @EVA Recheck prog_neo ??       
            
base_satellite_d1 <- left_join(d1,
            neorep_old_var) %>%
...
neorep_old_var %>% head()
base_satellite_d1 %>% group_by(str_TILs ) %>% count()

# base_satellite_d1 %>% group_by(tabac_3_classes) %>% count()
# base_satellite_d1 %>% group_by(smoking_3cl) %>% count()
# d1 %>% group_by(tabac_3_classes) %>% count()
# d1 %>% group_by(smoking_3cl) %>% count()
...
# base_satellite_d1$numdos7 == neorep$numdos