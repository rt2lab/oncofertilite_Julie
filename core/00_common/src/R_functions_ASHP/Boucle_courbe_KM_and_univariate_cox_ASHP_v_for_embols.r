...
library(survival)
source("/Users/ahamypet/RT2Lab/R/R functions/graph.surv_for_embols.r")
...
# 								categ <-levels(dataf[,quali[i]])
								...
								# Titre
								titre <- paste0(titre_var_surv , " by ", nomquali[i]," (", soustitre,")")
									  cat(i, "\n")
									  #i=3
# 										graph.surv(dataf[, del],dataf[, ev],gp=as.factor(dataf[ ,quali[i]]),main=titre,col="black",lty=1:9,lwd=1,xlab="months",ylab="%")
										...
										legend(2,0.3,legend_fig,col="black",lty=1:9,bty="o",cex=0.7)
  										text.default(20,0.35,labels=paste("LR test, p=",pval,sep="",collapse=NULL), cex=0.9)
