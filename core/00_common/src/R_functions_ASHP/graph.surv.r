# del : variable delais
# evt : variable evenement
# gp : variable de stratification (pour logrank eventuel)
# tps : sequence des temps e afficher sur le graphique et ds les donnees
# col : sequence des couleurs e attribuer aux courbes

# DESCRIPTION :
# trace une ou des courbes de survie en affichant sous le graphique les
# effectifs e risque e chaque temps

# pour faire apparaitre les IC: ajouter IC=c(), avec les temps oe l'on veut les IC dans les arguments de la fonction quand on la lance


graph.surv<-function(del,evt,     # del : variable delais;   evt : variable evenement
                     gp = 1,      # gp : variable de stratification (pour logrank eventuel)
                     tps = NULL,  # tps : sequence des temps e afficher sur le graphique et ds les donnees
                     col = 1:length(gp), lwd = 1, lty = 1, # col : sequence des couleurs e attribuer aux courbes
                     IC = NULL, decalIC = 0,
...

     {
            ...
            for (i in 1:length(tps)) {
              for (j in 1:nb) {
                text(tps[i], (-0.3 - 0.1 * j), expo[j, i], col = col[j], cex=0.8)  # Taille de police du nbre de sujets e risque : modifier cex ici
            ...
            if (!is.null(legend)) {
              cat("\n")
              cat("ou voulez-vous placer la legende e", "\n")
            ...
            if (nb > 1) {
              ...
              #mtext(paste("Logrank e ",nb-1," ddl:\n p = ",round(lr$p.value,3)),
