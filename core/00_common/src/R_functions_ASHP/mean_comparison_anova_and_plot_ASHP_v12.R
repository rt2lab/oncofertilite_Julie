...
# 														tata[2:length(tmp_level),c("tmp_variable","tmp_global_p_val")]		<- ""
														...
														tutu						<- cbind(   matrix(tmp_var_to_compare,nrow=length(tmp_variable), ncol=1 )	, matrix(tmp_name_var_to_compare,nrow=length(tmp_variable), ncol=1 )) 	
					...
					colnames(tab_reg_linear)			<- c("variable_coded","variable","variable_quali","levels","number_per_level","mean_value","pvalue vs ref class (t test)","pvalue ANOVA","pvalue ANOVA","post-hoc comp class","p val Tukey")