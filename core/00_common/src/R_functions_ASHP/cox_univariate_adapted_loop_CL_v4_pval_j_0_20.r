...
# i=10
   print(quali[i])
		...
		# remove if the only observation is NA for the outcome 
		tmp_tab_NA 		<- table(dataf[,quali[i]],exclude=NULL)		
 		toto 		<- table(dataf[, ev],dataf[,quali[i]],exclude=NULL)
		tmp_ncol	<-ncol(toto)-1
		titi		<- toto[which(is.na(rownames(toto))),-(tmp_ncol+1)		] 
		tata		<- tmp_tab	-  titi 

 		if(	! any(tata==0)	)  			{

		if(  (length(which(tmp_tab == 0)) != (length(tmp_tab)-1)  )		&    ! any(tmp_tab==0)			)	{
