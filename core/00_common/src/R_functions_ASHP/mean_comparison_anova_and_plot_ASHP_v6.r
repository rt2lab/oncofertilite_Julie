...
# 								names_variables_to_test		<- variables_to_test
		...
		for (k in 1:length(variable_to_compare)	) {
							...
							if( 	!any(tab==0)	 )	{
														...
														# Anova if more than 2 classes
														tmp_mod	 	<- aov(tmp_mat[,tmp_var_to_compare]	~ as.factor(tmp_mat[,tmp_var]))
...
# 														tmp_p_global				<- tmp_summary[1,"Pr(>F)"]
														tmp_p_global				<- tmp_summary[[1]][1,"Pr(>F)"]
# 														str()
