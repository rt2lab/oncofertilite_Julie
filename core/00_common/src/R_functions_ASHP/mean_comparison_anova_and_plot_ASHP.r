...
# 							pdf("results/description/TILs/mean_IT_TILs_value_quali_variables.pdf")
							...
							tab_reg_linear		<- NULL
								...
								for (i in 1:length(variables_to_test)	) {
														...
														print(tmp_name)
														tmp_mat		<- neorep_TILS_common[which(!is.na(neorep_TILS_common[,variable_to_compare]) & !is.na(neorep_TILS_common[,tmp_var])    ),c(tmp_var,variable_to_compare)]
							...
							if( 	!any(tab==0)	 )	{
										...
										ylim		<- range(neorep_TILS_common[,variable_to_compare])
