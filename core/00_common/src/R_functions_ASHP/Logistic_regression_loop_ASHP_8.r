...
# i=3
	...
	if(!any(tmp_tableau==0)  &  nrow(tmp_tableau) > 1  )		{
							...
							tmp_mod_null    				<- glm(as.numeric(dataf[,var_to_explain]) ~  1    , family="binomial")
							anova(tmp_mod,tmp_mod_null)
							anova(tmp_mod)
							summary(anova(tmp_mod))
							aov(tmp_mod,tmp_mod_null)
							aov(tmp_mod)
							
							# install.packages("car")
							Anova_cox_tmp_mod <-car::Anova(tmp_mod,test.statistic="F")
