		...
		for (k in 1:length(variable_to_compare)	) {
							...
							print(tmp_name_var_to_compare)												
								...
								for (i in 1:length(variables_to_test)	) {
														...
														print(tmp_name)
# 														tmp_mat		<- neorep_TILS_common[which(!is.na(neorep_TILS_common[,tmp_var_to_compare]) & !is.na(neorep_TILS_common[,tmp_var])    ),c(tmp_var,tmp_var_to_compare)]
