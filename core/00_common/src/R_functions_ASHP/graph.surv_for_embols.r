...
graph.surv<-function(del,evt,     # del : variable délais;   evt : variable événement
                     ...
                     col = 1:length(gp), lwd = 1, lty = 1, # col : séquence des couleurs à attribuer aux courbes
                     IC = NULL, decalIC = 0,
...
#####
# plot(toto)
# 			toto 		<- survfit( Surv(delDFS2.m, DFS)~EMBOLS_SEIN_DEF, data=neorep)
#  survfit
#  p_km <- plot(toto)
#  print(p_km)
# dev.off()
...
# 
# library(ggplot2)
# p		<- ggplot(data=neorep) + geom_boxplot(aes(x=RCH3.f,y=allreds.c))
# class(p)
# 
            ...
            if (!is.null(legend)) {
              cat("\n")
              cat("ou voulez-vous placer la legende ?", "\n")
