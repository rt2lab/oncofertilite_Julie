...
# 								names_variables_to_test		<- variables_to_test
		...
		for (k in 1:length(variable_to_compare)	) {
		  ...
		  print(k)
							...
							if( 	!any(tab==0)	&  length(rownames(tab))>1 )	{
...
# 														tata[2:length(tmp_level),c("tmp_variable","tmp_global_p_val")]		<- ""
					...
					tab_reg_linear[which(tab_reg_linear[,"pvalue ANOVA"]=="0"),"pvalue ANOVA"]  <- "<0.001"
					tab_reg_linear[which(tab_reg_linear[,"pvalue ANOVA2"]=="0"),"pvalue ANOVA2"]  <- "<0.001"
					tab_reg_linear[which(tab_reg_linear[,"p val Tukey"]=="0"),"p val Tukey"]  <- "<0.001"
