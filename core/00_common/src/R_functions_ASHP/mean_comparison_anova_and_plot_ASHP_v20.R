		  ...
		  print(k)
					...
					if (length(tmp_p_global)>0) {   # debut boucle si tmp_p_global existe
											...
											if (  tmp_p_global <= 0.05 & length(tmp_level) >2 ) {
														...
														tmp_median_bin       <- tmp_median$median
														
														# Here if we want to change the round for the mean !!
														titi 		<- rbind(tmp_variable,tmp_variable_unique,tmp_level,tmp_number,round(tmp_cum_value,1),round(tmp_pval,3) ,tmp_global_p_val,tmp_global_p_val_unique	, comp_class, pval_tukey, tmp_median_bin)
														tata		<- t(titi)
...
# Je viens de le rajouter la    														
														...
														colnames(toto)			<- c("variable_coded","variable","variable_quali","var_qual_unique","levels","number_per_level","mean_value","pvalue_vs_ref_class_t_test","pvalue_ANOVA","pvalue_ANOVA2","post_hoc_comp_class","p_val_Tukey","median","p_Kruskal","non_normal","effective_less_30")
														tab_reg_linear				<- rbind(tab_reg_linear, toto)

														# Rajouté ici
								colnames(tab_reg_linear)			<- c("variable_coded","variable","variable_quali","var_qual_unique","levels","number_per_level","mean_value","pvalue_vs_ref_class_t_test","pvalue_ANOVA","pvalue_ANOVA2","post_hoc_comp_class","p_val_Tukey","median","p_Kruskal","non_normal","effective_less_30")
