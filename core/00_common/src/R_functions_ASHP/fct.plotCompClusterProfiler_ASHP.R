...
require(plyr)
require(ggplot2)
GO.Changes <- c('anaphase-promoting complex-dependent proteasomal ubiquitin-dependent protein catabolic process' = 'catabolic process (2)',
				'positive regulation of protein modification by small protein conjugation or removal' = 'regulation of protein modification')
...
plotCompGO <- function(resComp, showCategory=5, grps=NULL,
	...
	######
	library(plyr)
	clProf.df$Description <- revalue(clProf.df$Description, replace=GO.Changes) 
