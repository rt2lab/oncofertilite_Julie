...
# 	
	...
	tmp_tab <- NULL
			...
			for (i in 1:length(quali)){
										...
										cox_p_coef		<-c("-",tmp_pvals)
										#cox_p_model		<-c("-",tmp_pvals)
										tmp_cox_p_model <- round(tmp_summary$waldtest[3],3)							# relabellisé p_global par  ASHP ; C'est celui selon le test de Wald
										cox_p_model		<-rep(tmp_cox_p_model, length(tmp_levels))									
										tmp_tab			<-cbind(tmp_var,tmp_levels,tmp_effectifs,tmp_events,HR,IC,cox_p_coef,cox_p_model)
