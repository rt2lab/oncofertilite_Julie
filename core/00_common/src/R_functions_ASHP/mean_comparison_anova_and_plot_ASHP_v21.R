		  ...
		  print(k)
							...
							print(tmp_name_var_to_compare)				# ESR1												
								...
								for (i in 1:length(variables_to_test)	) { # debut boucle variables_to_test
								            ...
								            print(i)
														...
														tmp_name 	<- names_variables_to_test[i]   ; print(tmp_name)		# LST_status 
					...
					if (length(tmp_p_global)>0) {   # debut boucle si tmp_p_global existe
											...
											if (  tmp_p_global <= 0.05 & length(tmp_level) >2 ) {
														...
														tmp_error               <- tmp_summary$coefficients[,2]
														tmp_ic_inf              <- round(tmp_value - 1.96*tmp_error,3)  
														tmp_ic_sup              <- round(tmp_value + 1.96*tmp_error,3)  
														tmp_ic_inf_sup          <- paste0("[",tmp_ic_inf,"-",tmp_ic_sup,"]") 
														tmp_ic_inf_sup[1]       <- "" 
														
														tmp_value_coef          <- unname(round(tmp_value,3))
														tmp_value_coef[1]       <- ""
														...
														tmp_mat2              <- tmp_mat
														colnames(tmp_mat2)    <- c("tmp_var","tmp_var_to_compare")
														tmp_median           <- tmp_mat2 %>% group_by(tmp_var) %>% summarise(median = median(tmp_var_to_compare, na.rm = TRUE))
														tmp_median_bin       <- round(tmp_median$median,1)
														
														# Here if we want to change the round for the mean !!
														titi 		<- rbind(tmp_variable,tmp_variable_unique,tmp_level,
														                tmp_number,round(tmp_cum_value,1),
														                tmp_value_coef,tmp_ic_inf_sup,
														                round(tmp_pval,3) ,tmp_global_p_val,tmp_global_p_val_unique	,
														                comp_class, pval_tukey, tmp_median_bin)
														tata		<- t(titi)
														tata[1,8]	<-	"reference class"		
# 														tata[2:length(tmp_level),c("tmp_variable","tmp_global_p_val")]		<- ""
														
														#####
														# loi normale?
														tmp_shapiro       <- shapiro.test(dataf[,tmp_var_to_compare]) #p-value = 2.208e-11
														tmp_pval_shapiro  <- tmp_shapiro$"p.value"                          
														tmp_non_normal    <- ifelse(tmp_pval_shapiro <0.05,"non-normal","")
														...
														if( 	any(tab<30)	 )	{
														  ...
														  tmp_effective_less_30 <- "effective less 30"
														                         }
														if( 	!any(tab<30)	& length(rownames(tab))>1   )	{
														  tmp_kruskal_pval    <- ""
														  tmp_effective_less_30 <- ""

														  if(tmp_pval_shapiro<0.05){
														    tmp_kruskal          <- kruskal.test(  tmp_mat[,tmp_var_to_compare]	~ as.factor(tmp_mat[,tmp_var])   )
														    tmp_kruskal_pval     <- round(tmp_kruskal$p.value,3)
														  }
...
# Je viens de le rajouter la    														
														...
														toto$non_normal        <- rep(tmp_non_normal, nrow(toto)) 
														toto$effective_less_30 <- rep(tmp_effective_less_30, nrow(toto)) 
														...
														colnames(toto)			<- c("variable_coded","variable","variable_quali","var_qual_unique",
														                        "levels","number_per_level","mean_value",
														                         "coefficient","CI",
														                        "pvalue_vs_ref_class_t_test",
														                      "pvalue_ANOVA","pvalue_ANOVA2","post_hoc_comp_class","p_val_Tukey",
														                        "median","p_Kruskal","non_normal","effective_less_30")
														tab_reg_linear				<- rbind(tab_reg_linear, toto)

														# Rajouté ici
								colnames(tab_reg_linear)			<- c("variable_coded","variable","variable_quali","var_qual_unique","levels","number_per_level","mean_value",
								                                "coefficient","CI",
								                                "pvalue_vs_ref_class_t_test","pvalue_ANOVA","pvalue_ANOVA2","post_hoc_comp_class","p_val_Tukey","median","p_Kruskal","non_normal","effective_less_30")
		...
		if( !is.null(tab_reg_linear)) {
		  ...
		  tab_reg_linear[which(tab_reg_linear[,"p_Kruskal"]==0)         ,"p_Kruskal"]                             <- "<0.001"
		}
		...
		tab_reg_linear <- tab_reg_linear %>% 
                          		            mutate(test              = case_when(
                          		                                                 non_normal        == "non-normal"       ~ "Kruskal" ,
                          		                                                 effective_less_30 == "effective less 30"~ "Kruskal",
                          		                                                 TRUE                                    ~ "ANOVA" ),
                          		                   p_Kruskal = as.character(p_Kruskal),
                                                 # pval_global_final = ifelse(p_Kruskal != "", p_Kruskal,pvalue_ANOVA) ,
                          		                   pval_global_final = case_when( non_normal       == "non-normal"        ~ p_Kruskal ,
                                                      		                     effective_less_30 == "effective less 30" ~ p_Kruskal, 
                                                      		                     TRUE                                     ~ pvalue_ANOVA ),
		...
		tab_reg_linear_short <- tab_reg_linear %>% select(var_qual_unique,levels,number_per_level,mean_value,
		                                                  ...
		                                                  post_hoc_comp_class,p_val_Tukey,median,p_Kruskal)
		tab_reg_linear_short_clean <- tab_reg_linear_short %>% 
		                        dplyr :: rename( variable = var_qual_unique ,
		                          ...
		                          select(variable, levels, n , mean, median, everything(), - p_Kruskal)
