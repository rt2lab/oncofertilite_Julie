...
for (i in 1:length(quali)) {
      ...
      tmp_tab_no_NA_for_variable		<- tmp_tab	-  titi 
        ...
        if(	! any(tmp_tab_no_NA_for_variable==0)	)  			{
                ...
                tab1 			    <- data.frame( "Variable"= nomquali[i], "Class"=levels(as.factor(dataf[,quali[i]]))[1], "HR"=1, "CI"=NA, "p_indiv2"=NA,"p_indiv3"=NA,  # Test indiv
                                              "p_log_rank2"=p_lr_test2, "p_log_rank3"=p_lr_test3,
                                               "p_Wald2"=p_Wald2, "p_Wald3"=p_Wald3) 

                tab2 			    <- data.frame( "Variable"=nomquali[i], 	"Class"=levels(as.factor(dataf[,quali[i]]))[-1], "HR"=RR, "CI"=IC, "p_indiv2"=p_indiv2,"p_indiv3"=p_indiv3 ,
                ...
                tab				      <- tab[,c("Variable","Var_unique", "Class", "Number","Events", "HR", "CI", "p_indiv3", "p_Wald3","p_log_rank3","p_indiv2","p_Wald2","p_log_rank2")] 			
                #
                 toto <- c("Variable","Var_unique", "Class", "Number","Events", "HR", "CI", "p_indiv3", "p_Wald3","p_log_rank3","p_indiv2","p_Wald2","p_log_rank2")
                 toto %in% colnames(tab)
                ...
                tabf 			<- rbind(tabf, tab)        
                                              ...
                                              colnames(tab) <-  c("Variable","Var_unique", "Class", "Number","Events", "HR", "CI", "p_indiv3", "p_Wald3","p_log_rank3","p_indiv2","p_Wald2","p_log_rank2") 			
                                              tabf 			<- rbind(tabf, tab)        
                                                                                                          }
                          }
              ...
              tabf           <- tabf[,c("Variable","Var_unique", "Class", "Number","Events", "HR", "CI",  "p_indiv3","p_Wald3","p_log_rank3","p_indiv2","p_Wald2","p_log_rank2")]   	
              ...
              colnames(tabf) <-  c("Variable_trash","Variable", "Category", "n","ev", "HR", "95%CI", "p*", "p_Wald3","p","p_indiv2","p_Wald2","p_log_rank2") 			
