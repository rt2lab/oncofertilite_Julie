...
source('~/RT2Lab/databases/core/00_common/src/R_functions_enora/mes_fonctions2.R', local  = TRUE)
...
var_quali <- as_tibble(d) %>% dplyr:: select(all_of(var)) %>% select_if(., is.factor) %>% names()

# DOnner les jolis noms de variables à partir de var
################################### TO DO   ################################### 
# pretty_var_quali <-  c("Menopausal status") #,"Molecular subtype","pCR")
pretty_var_quali <-  c("Subtype") #,"Molecular subtype","pCR")

d  <-  p53
  # droplevels(d[,,drop=FALSE])
...
tab_inter <- tab_inter_quali %>% mutate(category = as.factor(category))
...
# tablo_def_or %>% group_by(num_fig) %>% dplyr::summarize(plot = list(
  forestplot(
    # labeltext = label,
        labeltext = tablo_def_or$label,
tablo_def_or$number[, 1:3],
    # number[, 1:3],
    graph.pos = 6,
     clip = c(0.8, 10),
    #xlog = TRUE),
