...
#load("/Users/ahamypet/RT2Lab/databases/core/29_pregnancy_after_cancer/data/cohort_1_2_pat.RData")
#Comment Elise : je load ta database avec chemin relatif pour que ça fonnctionne chez tout le monde automatiquement 
load(file.path( 
     Sys.getenv("PROJECT_PATH"),
     "core/29_pregnancy_after_cancer/data/cohort_1_2_pat.RData"))

#Define your Mapping Class
Mappingpregnancy_after_cancer <- setClass( #TODO : change the name of the classe (e.g. MappingSein, MappingCanto...)
  "Mappingpregnancy_after_cancer", #TODO : same

  # Set the default values for the slots. (optional)
  prototype=list(
    name_database = "pregnancy_after_cancer", #TODO : the name of your base
    #TODO : load your database here e.g.
    database =  cohort_1_2_pat , 
    # database = read.csv(file = file.path( 
    # database = load(file.path( 
    #     Sys.getenv("PROJECT_PATH"),
    #     "core/29_pregnancy_after_cancer/data/raw/cohort_1_2_pat.RData")),
...
#Beware : you need to return the whole object of class Mappingpregnancy_after_cancer (self)
setMethod(f="mapping_initial",
          signature="Mappingpregnancy_after_cancer", #TODO : change to the name of your class
...
setMethod(f="mapping_patient_id",
          signature="Mappingpregnancy_after_cancer", #Change here
          definition=function(self)
          {
            ...
            dat_chir         <- as.Date(self@database$DATCHIReno)
            dat_bc_diagnosis <- as.Date(NA) 
            dat_bc_diagnosis[self@database$neoadj == "Yes"] <- dat_chir[self@database$neoadj == "Yes"] - 120
            dat_bc_diagnosis[self@database$neoadj == "No"]  <- dat_chir[self@database$neoadj == "No"] - 25

            return(list(
              database             = rep(29,nrow(self@database)),
              numdos_curie         = self@database$numdos7,
              cletri               = paste0("Pt_",c(1:nrow(self@database))),
              # side                 = self@database$side                ,
              dat_birth            = self@database$DDN   ,        
              dat_bc_diagnosis     = dat_bc_diagnosis      
              # dat_rando_inclusion  = self@database$dat_rando_inclusion ,  @ASHP to recup
...
setMethod(f="mapping_bc_diagnosis",
          ...
          definition=function(self)
          {
             ...
             ctuicc_5cl <- NA 
             ctuicc_5cl[self@database$T == "T0"]  <- 0
             ctuicc_5cl[self@database$T == "T1"]  <- 1
             ctuicc_5cl[self@database$T == "T2"]  <- 2
             ctuicc_5cl[self@database$T == "T3"]  <- 3
             ctuicc_5cl[self@database$T == "T4"]  <- 4
            ...
            return(list(
              ...
              cnuicc_2cl          = ifelse(self@database$N == "cN0",0,1)         
...
setMethod(f="mapping_bc_biology",
          ...
          definition = function(self)
          {
            ...
            er_status   <- ifelse(self@database$er == "positive", 1,0)
            pr_status   <- ifelse(self@database$pr == "positive", 1,0)
            her2_status <- ifelse(self@database$HER2 == "positive", 1,0)
            
            invasive_or_dcis <- ifelse(self@database$infilt %in% c("Invasive","invasive"),1,2)
            
            histo_3cl             <- NA 
            histo_3cl[self@database$typhist == "NST"]     <- 1
            histo_3cl[self@database$typhist == "lobular"] <- 2
            histo_3cl[self@database$typhist == "others"] <- 3
            ...
            return(list(
              ...
              histo_3cl            = histo_3cl        ,
              grade_3cl            = as.integer(self@database$gradeclasse)        ,
...
setMethod(f="mapping_surgery",
          ...
          definition=function(self)
          {
            breast_surgery_3cl   = ifelse(self@database$gestchirsein == "Lumpectomy",1,2)
            axillary_surgery_4cl = NA
            axillary_surgery_4cl[which(self@database$gestgg == "SND")] <- 1
            axillary_surgery_4cl[which(self@database$gestgg == "AND")] <- 2
            axillary_surgery_4cl[which(self@database$gestgg == "No axillary prelevement")] <- 9

            dat_chir         <- as.Date(self@database$DATCHIReno)

            return(list(
              breast_surgery_3cl   = breast_surgery_3cl  ,
              dat_first_surg       = dat_chir      ,
...
setMethod(f="mapping_treatments_binary",
          ...
          definition=function(self)
          {
            ct <- ifelse(self@database$ct == "Yes" , 1, 0)
            # rt <- ifelse(self@database$DATDRT
            ...
            return(list(
              ...
              ht                     = ifelse(self@database$ht == "Yes",1,0)  ,
              # dat_first_ht         = self@database$dat_first_ht         ,
              # ht_type_5cl          = self@database$ht_type_5cl          ,
              antiher2             = ifelse(self@database$typtc == "Trastuzumab", 1,0)
...
setMethod(f="mapping_neoadj_or_not",
          ...
          definition=function(self)
          {
            return(list(
              ...
              primary_ttt = ifelse(self@database$first_ttt == "Surgery",1,2)
...
setMethod(f="mapping_events_and_censor",
          ...
          definition=function(self)
          {
             return(list(
               ...
               dat_last_update      = self@database$DATDN    ,
               # ev_prog_neo          = self@database$ev_prog_neo        ,
               # dat_prog_neo         = self@database$dat_prog_neo       ,
               ev_recloc            = self@database$recid          ,
               dat_recloc           = self@database$DATRLOC.f1         ,
               ev_recreg            = self@database$recreg          ,
               dat_recreg           = self@database$DATRGG.f1       ,
               ev_meta              = self@database$meta            ,
               dat_meta             = self@database$DATMET.f1       ,  
