...
opt            <- NULL
opt$db_name    <- "31_tabac_survival" #TODO : the name of your database
opt$class_name <- "MappingTabacSurvival" #TODO : the name of your class
opt$output_folder <- ""
Sys.setenv("PROJECT_PATH"="/Users/elisedumas/Code/databases") #TODO : your PROJECT_PATH
...
load(file.path(output_folder, "/31_tabac_survival_mapped.RData"), verbose=TRUE)
self <- db
...
########################################################################
#Step 6 : check your preprocessing
#Option 2 : run only one method

#In this case, use the mapped database (because of dependencies which may exist with previous family var)
load(file.path(output_folder, "/31_tabac_survival_preprocessed_labels.RData"), verbose=TRUE)
summary(database_preprocessed_labels)

surv_var <- c(data_dict$var[data_dict$family_var %in% c("events_and_censor","evol")], "numdos_curie")
surv_var <- surv_var[!grepl("diag",surv_var)] #Les variables avec diag n'ont aucun sens (car j'ai mis date de chirurgie)
database_preprocessed_labels <- database_preprocessed_labels %>% 
  dplyr::select(one_of(surv_var))

#Save RData and csv
save(database_preprocessed_labels,file  = file.path(output_folder, "/31_tabac_survival_preprocessed_labels_survival.RData"))
write.csv(database_preprocessed_labels,file.path(output_folder, "/31_tabac_survival_preprocessed_labels_survival.csv"))
########################################################################
