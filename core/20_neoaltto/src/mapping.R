...
load( paste0(Sys.getenv("PROJECT_PATH"), "/core/20_neoaltto/data/neoaltto.RData"))

#Step 1
#Define your Mapping Class
MappingNeoAltto <- setClass( #TODO : change the name of the classe (e.g. MappingSein, MappingCanto...)
  "MappingNeoAltto", #TODO : same

  # Set the default values for the slots. (optional)
  prototype=list(
    name_database = "20_neoaltto", #TODO : the name of your base
...
setMethod(f="mapping_initial",
          signature="MappingNeoAltto", #TODO : change to the name of your class
...
setMethod(f="mapping_patient_id",
          signature="MappingNeoAltto", #Change here
          definition=function(self)
          {
            ...
            return(list(
               database             = rep(20,nrow(self@database)),
              # numdos_curie         = self@database$NUMDOS,
               cletri               = self@database$pt ,
              ...
              numdos_other_center    = self@database$pt 
            ))
          }
)
...
setMethod(f="mapping_patient_char",
          ...
          {
            return(list(
               age                  = as.numeric(self@database$age),
              ...
              # breast_feed         = self@database$breast_feed   ,
                menop               = regroup(self@database$menost,list(c("PREMENOPAUSAL","NA AND AGE<50"),c("POSTMENOPAUSAL","NA AND AGE>=50")), c(0,1)),
              ...
              # brca_1_2_mut        = self@database$brca_1_2_mut  ,
                weight              = as.numeric(self@database$wt),
                size                = as.numeric(self@database$ht)/100
          ...
          definition=function(self)
          {
            return(list(
              ...
              # moddiag             = self@database$moddiag            ,
                multifocality_clin  = factor(self@database$FOCALYN, levels = c( "YES","NO"),labels=c(1,0)),
                #tclin               = as.numeric(self@database$RTmSz)*10#,
                tclin               = as.numeric(self@database$test_size)*10#,
...
setMethod(f="mapping_bc_biology",
          ...
          {
            # her2_status                                                                                                                                   <- NA
            # her2_status[self@database$IHC_Results == "NEGATIVE" | self@database$FISH_Results == "NOT AMPLIFIED"]                                          <- 0
            # her2_status[self@database$IHC_Results == "POSITIVE" | self@database$FISH_Results == "AMPLIFIED" | self@database$CISH_Results == "AMPLIFIED"]  <- 1
            return(list(
              ...
              # pr_allred            = self@database$pr_allred        ,
               her2_status           = factor(self@database$her2_status),
              # histo_5cl            = self@database$histo_5cl        ,
               grade_3cl             = regroup(self@database$grade, list(c("G1"),c("G2"),c("G3"),c("GX","NK")),c(1,2,3,NA)),   
              ...
              # p53                  = self@database$p53              ,
               str_til_perc         = as.numeric(self@database$str_mean) ,  # mail Marick Lae, 21/03/25, averaging TILs from 2 anapath
               it_til_perc          = as.numeric(self@database$it_mean)  ,  # mail Marick Lae, 21/03/25, averaging TILs from 2 anapath
              # tumor_cellularity    = self@database$tumor_cellularity,  
               lvi_biop              = factor(self@database$lympyn, levels = c("NO","YES"),labels=c(0,1)),
               er_status_1_perc      = factor(self@database$ERCat,  levels = c("NEGATIVE","POSITIVE"),labels=c(0,1)),
               pr_status_1_perc      = factor(self@database$PgRCat, levels = c("NEGATIVE","POSITIVE"),labels=c(0,1))
            ))
          }
)
          ...
          definition=function(self)
          {
            return(list(
              ...
              # dat_end_first_ct     = self@database$dat_end_first_ct     ,
                rt                   = factor(self@database$n_rth, levels = c("0","1"),labels=c(0,1)),  
              # dat_first_rt         = self@database$dat_first_rt         ,
                ht                   = factor(self@database$n_hth, levels = c("0","1"),labels=c(0,1)), 
              # dat_first_ht         = self@database$dat_first_ht         ,
               ht_type_5cl           = factor(self@database$ht_type_5cl) 
...
setMethod(f="mapping_neoadjuvant_ct_antiher2",
          ...
          definition=function(self)
          {
            neo_incomplete_scheme                                                                                                         <- rep(NA,nrow(self@database))
            neo_incomplete_scheme[self@database$cmplyn1 == "YES" | self@database$cmplyn2 == "YES"]                                        <- 0
            neo_incomplete_scheme[self@database$cmplyn1 == "NO" | self@database$cmplyn2 == "NO"]                                          <- 1
            ...
            return(list(
              ...
              # neo_antiher2_regimen   = self@database$neo_antiher2_regimen
               neo_incomplete_scheme   = factor (neo_incomplete_scheme), 
               neo_trastu              = regroup (self@database$armdesc, list(c("LAPATINIB ALONE"),c("TRASTUZUMAB ALONE","LAPATINIB IN COMBINATION WITH TRASTUZUMAB")),c(0,1)), 
               neo_lapatinib           = regroup (self@database$armdesc, list(c("TRASTUZUMAB ALONE"),c("LAPATINIB ALONE","LAPATINIB IN COMBINATION WITH TRASTUZUMAB")),c(0,1))
               ))
          }
)
...
setMethod(f="mapping_tumor_char_neo",
          ...
          definition=function(self)
          {
            ...
            # pcr[which(breast_res_infiltr == 1 & self@database$nuicc_4cl==0)]  <- 1 #A revoir
            ...
            return(list(
              ...
              # breast_res_infiltr         = factor(self@database$PCR4FL, levels = c("Y","N"),labels=c(0,1)), 
               #ypnuicc_2cl                = factor(self@database$PCR4FL, levels = c("Y","N"),labels=c(0,1))
               pcr = factor(self@database$PCR4FL, levels = c("N","Y"),labels=c(0,1))# 
...
setMethod(f="mapping_events_and_censor",
          ...
          {
             return(list(
                dat_censor_database  =  as.numeric(self@database$AVAL.x)*12,
                delay_efs_diag       =  as.numeric(self@database$AVAL.x)*12,
               # dat_last_update      = self@database$dat_last_update    ,
                ev_prog_neo           = factor (self@database$ ev_prog_neo),
               # dat_prog_neo         = self@database$dat_prog_neo       ,
                ev_recloc            = factor (self@database$ev_recloc )         ,
               # dat_recloc           = self@database$dat_recloc         ,
               # ev_recreg            = self@database$ev_recreg          ,
               # dat_recreg           = self@database$dat_recreg         ,
                ev_meta              = factor (self@database$ev_meta)            ,
               # dat_meta             = self@database$dat_meta           ,
                ev_contro            = factor (self@database$ev_contro )         ,
               # dat_contro           = self@database$dat_contro         ,
                ev_secondk           = factor (self@database$ev_secondk)         ,
               # dat_secondk          = self@database$dat_secondk        ,
                delay_os             =  as.numeric(self@database$AVAL.y)*12,
                status_vital         = factor(self@database$EVNT.y)      # ,
               # cause_death           = self@database$EVNTDESC        ,
