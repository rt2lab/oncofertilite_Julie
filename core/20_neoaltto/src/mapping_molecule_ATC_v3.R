...
#############################################################################################################################################################################
#### 1. Load list of virgin molecules and medicinal history #################################################################################################################
#############################################################################################################################################################################

patients_concomit <- read_sas(file.path(DATA_PATH, "dataset4_concomitant.sas7bdat"), NULL)  # 7,781 x 3
  dim(patients_concomit) #[1] 7781    3
  colnames(patients_concomit) #  "pt"    "trpt"  "trsct"
  head(patients_concomit)
  patients_concomit_unduplicated <- patients_concomit %>% distinct(pt,trpt)  
  dim(patients_concomit_unduplicated) #[1] 5780    2
  #View (patients_concomit_unduplicated)
...
#############################################################################################################################################################################
#### 2. Load list of virgin molecules separated by SPLIT ####################################################################################################################
#############################################################################################################################################################################
 
patients_concomit_split   <-read.csv ("/Users/beatrizgrandal/GitHub/databases/core/20_neoaltto/data/comedic_ATC/raw_nadir_all/dataComeds.csv", header = TRUE, sep = ';')
dim (patients_concomit_split) #[1] 8810   13
colnames(patients_concomit_split)
# [1] "X"                      "pt"                     "trpt"                   "trsct"                  "medication_name"        "NAME_minimal_cleanin"   "NAME_parsed_nb"         "NAME_parsed_first_word"
# [9] "ATC"                    "source"                 "multipleATC"            "mappingMode"            "ATC_mapped_CANTO" 
nrow(patients_concomit_split_unduplicated <- patients_concomit_split %>% distinct(pt,trpt))  # [1] 6556

head(patients_concomit_split_unduplicated)
#View (patients_concomit_split_unduplicated)


length(unique(patients_concomit_split_unduplicated $trpt)) #[1] 1086


n_patients_concomit_split_unduplicated <- patients_concomit_split_unduplicated %>% 
  dplyr::group_by(pt) %>% 
...
dim(whole_name_medical) #[1] 409   3
head(whole_name_medical)
#View (whole_name_medical)
...
dim (mapped_first_word_medical) #[1] 178   3
head(mapped_first_word_medical)
...
nrow(not_mapped_medical %>% distinct(trpt)) #[1] 280
table( not_mapped_medical$ATC, exclude = NULL) # NA 164 


####################### ****  Multiple ATC mapping **** ########################################### 
dim( multiple_ATC_unique) #[1] 223  16
nrow (multiple_ATC_unique)
head(multiple_ATC_unique)
multiple_ATC_unique_not_repeat <- multiple_ATC_unique%>% distinct(trpt) #[1] 220
...
dim  (multiple_ATC_not_relevant) #[1] 110   3
nrow (multiple_ATC_not_relevant  )
head (multiple_ATC_not_relevant  )
#View(multiple_ATC_not_relevant )
#nrow (multiple_ATC_not_relevant  %>% distinct(trpt))  #1] 124


multiple_ATC_relevant  <- multiple_ATC_unique  %>% 
  filter (medicinal_relevant == "1, Yes")  %>% distinct(trpt)# %>% nrow()  # 109 -3 -- > 106

dim(multiple_ATC_relevant ) #[1] 109   1
...
multiple_ATC_disease2 %>% distinct(trpt)
...
atc_mapping_molecule <-do.call(rbind,list(whole_name_medical,mapped_first_word_medical, not_mapped_medical,multiple_ATC_disease, multiple_ATC_unique2, multiple_ATC_not_relevant ))
...
dim(atc_mapping_molecule ) #[1] 1086    3 --> # !!!!! Attention,  Here there are NEOMYCIN and NEOMYCIN SULFAT
#View (atc_mapping_molecule)
...
# ****************************************************************************
######################################################################################################################################
### To make EXCEL to choose ATCs in those molecules with multiple ATCs and medical history necesary ############################################################
######################################################################################################################################
  # **************************
...
###################################################################################
### Mapping ATC UNIQUE DRUGS to BASE DATASET with ALL MOLECULE##################
#################################################################################
  
dim(patients_concomit_split_unduplicated ) #[1] 6556    2
#View(patients_concomit_split_unduplicated )
colnames (patients_concomit_split_unduplicated) #"pt"   "trpt"

dim (atc_mapping_molecule) #[1] [1] 1086    3
#View(atc_mapping_molecule) 
colnames (atc_mapping_molecule)    #"trpt"   "ATC"    "source"

# Find the differences 
 setdiff(atc_mapping_molecule$trpt,patients_concomit_split_unduplicated$trpt)  #[1] "character(0)  
 setdiff(patients_concomit_split_unduplicated$trpt,atc_mapping_molecule$trpt)  #[1] "character(0)  

 atc_mapping_molecule_tmp  <- patients_concomit_split_unduplicated  %>% 
   anti_join (atc_mapping_molecule, by = c("trpt" = "trpt"))
 dim(atc_mapping_molecule_tmp) #[1] 0  2

tmp_repeat <- atc_mapping_molecule %>% group_by(trpt) %>% filter(n()>1) %>%  arrange(trpt)  #--> 0
 #View (tmp_repeat )
 
 
 # Mix 
    atc_mapping_molecule2  <- patients_concomit_split_unduplicated  %>% 
      left_join (atc_mapping_molecule, by = c("trpt" = "trpt"))
  dim(atc_mapping_molecule2) #[1] 6556    4  
  View (atc_mapping_molecule2 )

    # not repeated drugs 
    tmp_atc_mapping_molecule3 <- distinct(atc_mapping_molecule2)
    nrow (tmp_atc_mapping_molecule3) #[1] 6556
...
#View (multiple_atc_unique_with_medical_history)
#multiple_atc_unique_with_medical_history$final_atc_5
dim(multiple_atc_unique_with_medical_history) #[1] 615   8
...
atc_mapping_molecule_end  <- atc_mapping_molecule2 %>% 
  left_join(multiple_atc_unique_with_medical_history, by= c("pt", "trpt" )) 
            #by = c("pt" = "pt")) #%>% 
dim(atc_mapping_molecule_end) #[1] 6556   10
...
###### LAST VERSION ####### 
# direct_atc     first_word_medical          manual_mapped multiple_atc_discarded   multiple_atc_disease    multiple_atc_unique 
# 2932                    875                    909                    672                    680                    488 
...
# Save NA
write.csv (na_atc, file="/Users/beatrizgrandal/GitHub/databases/core/20_neoaltto/data/comedic_ATC/final_v1/dataComeds_na_atc.csv")
...
dim(base_comedic) #[1] 5366    2
View (base_comedic)

# Remove ATC duplicated 
nrow(base_comedic <- distinct(base_comedic)) # [1] 5212   
base_comedic <- distinct(base_comedic)
dim(base_comedic)
...
ttconc_chronic = l[[1]]
View (ttconc_chronic)
dim(ttconc_chronic) #[1] 3289    2
nrow(casa <- distinct(ttconc_chronic)) #[1] 3289

ttconc_chronic <- as.data.frame(ttconc_chronic)
ttconc_removed = l[[2]]
dim(ttconc_removed) #[1] 1923    2
