...
load(paste0(Sys.getenv("PROJECT_PATH"),"/core/28_neoadj_sls/data/new_base_sls.RData") )
# dim(new_base_sls)
library(dplyr)
#new_base_sls <- new_base_sls %>% select(-str_til_perc,-str_til_perc_postneo)
new_base_sls <- new_base_sls %>% select(-str_til_perc_postneo)

#Step 1
#Define your Mapping Class
MappingNeo_sls <- setClass( #TODO : change the name of the classe (e.g. MappingSein, MappingCanto...)
  "MappingNeo_sls", #TODO : same

  # Set the default values for the slots. (optional)
  prototype=list(
    name_database = "neoadj_sls", #TODO : the name of your base
    #TODO : load your database here e.g.
    # database = read.csv(file = file.path(
    database = new_base_sls, 
...
#Beware : you need to return the whole object of class MappingNeo_sls (self)
setMethod(f="mapping_initial",
          signature="MappingNeo_sls", #TODO : change to the name of your class
...
setMethod(f="mapping_patient_id",
          signature="MappingNeo_sls", #Change here
          definition=function(self)
          {
            ...
            return(list(
              database             = rep(28,nrow(self@database)),
              # numdos_other_center  = self@database$NIP,
              cletri               = paste0("Pt_",c(1:nrow(self@database))),
              side                 = self@database$side                ,
              dat_birth            = self@database$dat_birth           ,
              dat_bc_diagnosis     = self@database$dat_first_biopsy    
...
setMethod(f="mapping_patient_char",
          ...
          definition=function(self)
          {
            return(list(
              ...
              # hormo_contra         = self@database$hormo_contra   ,
              # hormo_contra_year    = self@database$hormo_contra_year      ,
              menop               = self@database$menop         ,
              # age_menop           = self@database$age_menop     ,
              hrt                 = self@database$hrt           ,
              # nb_fam_history      = self@database$nb_fam_history  ,
...
setMethod(f="mapping_tumor_char_neo",
          ...
          definition=function(self)
          {
            return(list(
              ...
              # nbggprel_postneo          = self@database$nbggprel_postneo
...
#
# #pre_post_neo
# setMethod(f="mapping_pre_post_neo",
#           signature="MappingNeo_sls", #TODO : change to the name of your class
#           definition=function(self)
#           {
#             return(list(
